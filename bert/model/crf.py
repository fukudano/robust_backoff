...
class CRF(nn.Module):
    def __init__(self, tag_stoi, gpu=True):
        super().__init__()
        self.gpu = gpu
        # Matrix of transition parameters.  Entry i,j is the score of transitioning from i to j.
        self.tagset_size = len(tag_stoi)
        # # We add 2 here, because of START_TAG and STOP_TAG
        # # transitions (f_tag_size, t_tag_size), transition value from f_tag to t_tag
        NINF = -1e5
        init_transitions = torch.zeros(self.tagset_size + 2, self.tagset_size + 2)
        init_transitions[:, START_TAG] = NINF
        init_transitions[STOP_TAG, :] = NINF
        if "O" in tag_stoi:
            for to in tag_stoi.items():
                for fr in tag_stoi.items():
                    if fr != to:
                        if to[0].startswith("I-") and fr[0][2:] != to[0][2:]:
                            init_transitions[fr[1], to[1]] = NINF
                if to[0].startswith("I-"):
                    init_transitions[START_TAG, to[1]] = NINF
        if self.gpu:
