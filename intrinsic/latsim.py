...
class LatSim:
    def __init__(self, words, measure, gpu):
        ...
        self.simstring = Simstring(self.words, measure=measure, be=True, unicode=True, file=f"{gpu}.db")
    ...
    def ret(self, w, k, lb=0.0, same=False):
        rs = self.simstring.topk(w=w, k=k, lb=lb, same=same)
        return rs
